...
function handleInputChange() {
    if (searchQuery.value === "") {
        Inertia.get(route("admin.doctors"), {}, { preserveState: true });
    }
}
...
const currentDoctor = ref(null); // track current doc
...
const openEditDoc = (doctor) => {
    ...
    // Pre-fill form
    form.docimage = null;
    form.docroom = doctor.docroom;
    form.docname = doctor.docname;
    form.docspec = doctor.docspec;
    form.docphone = doctor.docphone;
    form.docdep = doctor.docdep;
};
...
const submit = () => {
    if (isAddDoc.value) {
        ...
        // Handle edit user
        form.post(route("doctor.update", currentDoctor.value.id), {
            onSuccess: () => {
                closeModal();
                Swal.fire({
                    title: "Success!",
...
const deleteDoctor = (doctor) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.delete(route("doctor.destroy", doctor.id), {
                onSuccess: () => {
                    Swal.fire({
                        title: "Deleted!",
...
<template>
    ...
    <el-dialog
        ...
        :title="isAddDoc ? 'Add Doctors' : 'Edit Doctors'"
        width="500"
    ...
    >
       ...
       >
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <InputError class="mt-1" :message="form.errors.docdep" />
            </div>
