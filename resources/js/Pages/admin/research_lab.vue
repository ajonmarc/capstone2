...
function handleInputChange() {
    if (searchQuery.value === "") {
        Inertia.get(route("admin.research_labs"), {}, { preserveState: true });
    }
}
...
const isAddRes = ref(false);
const dialogVisible = ref(false);
const currentRes = ref(null); // track current doc
...
const openEditRes = (research_lab) => {
    isAddRes.value = false;
    dialogVisible.value = true;
    currentRes.value = research_lab;

    // Pre-fill form
    form.image = null;
    form.name = research_lab.name;
    form.description = research_lab.description;
};
    ...
    if (isAddRes.value) {
        ...
        form.post(route("research.store"), {
            onSuccess: () => {
                ...
                Swal.fire({
                    title: "Success!",
                    text: "Research lab has been created successfully.",
                    icon: "success",
                    toast: true,
                    position: "bottom-end",
        ...
        // Handle edit user
        form.post(route("research.update", currentRes.value.id), {
            onSuccess: () => {
                closeModal();
                Swal.fire({
                    title: "Success!",
                    text: "Research lab has been updated successfully.",
                    icon: "success",
                    toast: true,
                    position: "bottom-end",
                    showConfirmButton: false,
...
const deleteRes = (research_lab) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.delete(route("research.destroy", research_lab.id), {
                onSuccess: () => {
                    Swal.fire({
                        title: "Deleted!",
                        text: "Research lab has been deleted.",
                        icon: "success",
                        toast: true,
                        position: "bottom-end",
                        showConfirmButton: false,
...
<template>
    ...
    <el-dialog
        ...
        :title="isAddRes ? 'Add Research lab' : 'Edit Research lab'"
        width="500"
                ...
                >
                    ...
                    <div class="overflow-x-auto">
                        ...
                        >
                            ...
                            <tbody>
                                <tr v-if="research_labs.data.length === 0">
                                    <td
                                        colspan="5"
                                ...
                                <tr
                                    v-for="research_lab in research_labs.data"
                                    :key="research_lab.id"
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3">
                                        <!-- Edit button -->
                                        <button
                                            @click="openEditRes(research_lab)"
                        ...
                        <!-- Pagination Links -->
                        <div v-if="research_labs.links.length > 1">
                            <nav
